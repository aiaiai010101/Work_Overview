...
def render_single_card(item):
    ...
    with ui.card().classes('w-full') as card:
        ...
        ui.separator()
        ui.label(item['content']).style('white-space: pre-wrap; line-height: 1.8;')
    return card


def render_all_cards(items_list):
    """渲染全部卡片，返回 {id: card} 字典"""
    sections = {}
    for item in items_list:
        sections[item['id']] = render_single_card(item)
    return sections
...
def on_tree_select(ref, e):
    ...
    if target_id and target_id in ref['detail_sections']:
        # 切到详情容器，滚动到目标卡片
        ref['list_container'].set_visibility(False)
        ref['detail_container'].set_visibility(True)
        card = ref['detail_sections'][target_id]
        ui.run_javascript(
...
def back_to_list(ref):
    ref['detail_container'].set_visibility(False)
    ref['list_container'].set_visibility(True)


def on_search(ref, e):
    keyword = e.value.strip().lower() if e.value else ''
    back_to_list(ref)
    for item_id, card in ref['list_sections'].items():
...
def index():
    ...
    with ui.splitter(value=25).classes('w-full').style(
        ...
        with splitter.after:
            ...
            # 容器1：列表视图（默认显示，自由滚动）
            ref['list_container'] = ui.scroll_area().classes('w-full').style(
                'height: calc(100vh - 52px);'
            )
            with ref['list_container']:
                with ui.column().classes('w-full gap-6 p-6'):
                    ref['list_sections'] = render_all_cards(ITEMS_DATA)

            # 容器2：导航视图（默认隐藏，点树节点切过来并定位）
            ref['detail_container'] = ui.scroll_area().classes('w-full').style(
                'height: calc(100vh - 52px);'
            )
            ref['detail_container'].set_visibility(False)
            with ref['detail_container']:
                with ui.column().classes('w-full gap-4 p-6'):
                    ui.button(
                        '← 返回列表',
                        on_click=lambda: back_to_list(ref),
                    ).props('flat color=primary')
                    with ui.column().classes('w-full gap-6'):
                        ref['detail_sections'] = render_all_cards(ITEMS_DATA)
